...
export default async (event) => {
  try {
    const tag = pickTopic(event);
    if (!tag) return html(302, "", { Location: "/topics.html" });

    const premium = await getPremiumTokenRecord(event);

    const { rows } = await query(
      `
      SELECT d.slug, d.title, d.source_name, d.source_url, d.published_at, d.image_url, d.excerpt, d.public_summary
      FROM documents d
      WHERE EXISTS (SELECT 1 FROM unnest(d.tags) t WHERE lower(t)=lower($1))
    ...
    const desc = `Themenseite: "${tag}". ${total} Treffer im Index aus öffentlich zugänglichen Quellen. Premium: Highlights, Smart‑Match, Alerts.`;

    const siteBase = process.env.URL || process.env.DEPLOY_PRIME_URL || "";
    const og = siteBase
      ? new URL(`/.netlify/functions/og?title=${encodeURIComponent(tag)}&subtitle=${encodeURIComponent("TOPIC • PROGRAMMATIC SEO")}`, siteBase).toString()
      : `/.netlify/functions/og?title=${encodeURIComponent(tag)}&subtitle=${encodeURIComponent("TOPIC • PROGRAMMATIC SEO")}`;
...
<main class="container">
  <div class="card pad" style="margin-top:16px">
    ...
    <div class="h1">${escapeHtml(tag)}</div>
    ...
    <div class="searchbar">
      <input value="${escapeHtml(tag)}" readonly>
      <a class="btn" href="/search.html?tag=${encodeURIComponent(tag)}">Suche mit Tag</a>
      <a class="btn" href="/alerts.html?q=${encodeURIComponent(tag)}">🔔 Alert setzen</a>
  ...
  <div class="grid" style="grid-template-columns:1.15fr .85fr;margin-top:18px">
    ...
    <aside>
      <div class="card pad">
        ...
        <div class="p muted">Smart‑Match & Highlights zeigen dir in Sekunden, ob es sich lohnt zu klicken.</div>
        <ul>
          <li><strong>Highlights:</strong> relevante Passagen & Prüfpunkte</li>
          <li><strong>Related Stories:</strong> ähnliche Fundstellen</li>
          <li><strong>Alerts:</strong> Mail, wenn Neues auftaucht</li>
        </ul>
        <a class="btn danger" href="#" data-subscribe>Freischalten ab €1,99</a>
        <hr>
        <div class="notice"><strong>Pro‑Move:</strong> Teilen → OG‑Bild sieht wie eine Akte aus (hohe CTR).</div>
      </div>

      ${locked ? `
      <div class="premiumLock" style="margin-top:14px">
        <h4>Premium‑Teaser (vernebelt)</h4>
        <div class="p small muted">Du siehst Kontext, Details bleiben hinter der Paywall.</div>
        <div style="filter: blur(7px); user-select:none; pointer-events:none; opacity:.9">
          <ul>
            <li>Querverweise zu anderen Topics</li>
            <li>Hinweise: welche Quellen widersprechen</li>
            <li>Lesereihenfolge: Was zuerst lesen</li>
          </ul>
        </div>
        <a class="btn danger" href="#" data-subscribe>Freischalten ab €1,99</a>
      </div>` : ""}
...
`});
    ...
    return html(200, page({
      title: `${tag} – Topic`,
      description: desc,
      urlPath: `/topic/${encodeURIComponent(tag)}`,
      ogImage: og,
      bodyHtml: body,
